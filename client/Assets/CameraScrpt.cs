...
public class CameraStreamer : MonoBehaviour
{
    ...
    IEnumerator SendFrameToServer()
    {
        ...
        Debug.Log("JSON Body: " + jsonBody);  // Add this log to check the JSON body

        // Create a UnityWebRequest to send the image data to the server
        using (UnityWebRequest www = new UnityWebRequest("http://localhost:5001/image/", "POST"))
        {
            byte[] jsonToSend = new UTF8Encoding().GetBytes(jsonBody);
            Debug.Log("JSON to Send Length: " + jsonToSend.Length);  // Check the length of the payload
